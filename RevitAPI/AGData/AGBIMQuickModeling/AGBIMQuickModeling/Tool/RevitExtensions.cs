...
namespace CADReader
{
    static class RevitExtensions
    {
        ...
        /// 转换为list curves 列表
        /// </summary>
        /// <param name="curveModels"></param>
        /// <returns></returns>
        public static List<Curve> ToCurves(this List<CADCurveModel> curveModels)
        {
            List<Curve> curves = new List<Curve>();
            foreach(var item in curveModels)
            {
                foreach(var _cur in item.Curves)
                {
                    curves.Add(_cur);
                }
            }
        ...
        /// <returns></returns>
        public static string ToNumberStr(this string keyword)
        {
            return System.Text.RegularExpressions.Regex.Replace(keyword, @"[^0-9]+", "");
        }

        /// <summary>
        /// 组合字符串
        /// </summary>
        public static string AddHead(this string value, string head)
        {
            return head + value;
        }
        ...
        /// <returns></returns>
        public static List<CADCurveModel> GroupCurves(this List<Curve> curves, double ignore = 0.001)
        {
            List<CADCurveModel> curvesModels = new List<CADCurveModel>();
            ...
            while(curves.Count > 0)
            {
                ...
                if(shape.Count > 1)
                {
                    curvesModels.Add(new CADCurveModel() { Curves = shape });
                }
            }
            return curvesModels;
        }
