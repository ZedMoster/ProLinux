...
namespace CADReader
{
    class ShareParatemer
    {
        ...
        public ShareParatemer(string sharedParametersFilename = "AGKeys.txt", string groupName = "AGKeys", string type=".txt")
        {
            var path = System.Environment.GetEnvironmentVariable("TEMP");
            if(path == null)
            {
                "Class ShareParatemer-参数系统缓存txt文件创建失败".ToDebug();
