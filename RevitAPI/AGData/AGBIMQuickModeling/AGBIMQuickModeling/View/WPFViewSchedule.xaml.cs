    ...
    public partial class WPFViewSchedule : Window
    {
        ...
        private List<ScheduleDatas> SchedulesToGruop(List<CADTextModel> texts, List<Line> lines)
        {
            ...
            foreach (var gp in groupInfo)
            {
                ...
                //var keystr = key.Replace("楼", "").Replace("#", "").Replace("栋", "");
                var keystr = key.ToNumberStr();
                ...
                try
                {
                    ...
                    {
                        Index = index,
                        Key_1 = gpSort.ElementAtOrDefault(0)?.Text ?? "N/A",
                        Key_2 = gpSort.ElementAtOrDefault(1)?.Text ?? "N/A",
                        Key_3 = gpSort.ElementAtOrDefault(2)?.Text ?? "N/A",
                        Key_4 = gpSort.ElementAtOrDefault(3)?.Text ?? "N/A",
                        Key_5 = gpSort.ElementAtOrDefault(4)?.Text ?? "N/A",
                        Key_6 = gpSort.ElementAtOrDefault(5)?.Text ?? "N/A",
                        Key_7 = gpSort.ElementAtOrDefault(6)?.Text ?? "N/A",
                        Key_8 = gpSort.ElementAtOrDefault(7)?.Text ?? "N/A",
                        Key_9 = gpSort.ElementAtOrDefault(8)?.Text ?? "N/A",
                        Key_10 = gpSort.ElementAtOrDefault(9)?.Text ?? "N/A",
                    });
                }
                catch (Exception e)
                {
