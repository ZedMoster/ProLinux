...
namespace hybh
{
    ...
    class AlignColumnTopWithFloor : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            if (Run.Running(Strings.key))
            {
                ...
                if (elementColumns.Count == 0)
                {
                    ...
                    {
                        // 选择过滤器
                        var filter_Column = new PickByCategorySelectionFilter() { CategoryName = "结构柱" };
                        // 右键完成多选
                ...
                foreach (var el in elementColumns)
                {
                    ...
                    if (result0 != null)
                    {
                        ...
                        // 待修改柱参数 顶部偏移 FAMILY_TOP_LEVEL_OFFSET_PARAM
                        var _Parameter = el.get_Parameter(BuiltInParameter.FAMILY_TOP_LEVEL_OFFSET_PARAM);
